...
- `app/(app)/people/page.tsx` — server component listing groups and people: a Groups section (with GroupForm and DeleteGroupButton) and a People section (with PeopleForm and RemoveContactButton). Row actions when `pair.amountOwedToMe.gt(0)`: `RemindButton` (counterparty registered only), `WhatsAppButton` (phone present; precomputed via `getPairDetail`), otherwise an inline `AddPhoneForm`; a `Details` link always goes to `/people/{id}`.
- `app/(app)/people/[userId]/page.tsx` — pair detail (server component): headline (owes you / you owe / All settled) in accent/destructive/muted, itemized expense list with 'You paid'/'They paid' + share amount, the same action buttons, and a per-sender `MarkRemindersReadButton` when an unread reminder exists. Redirects to `/people` when the pair has no outstanding split or the user doesn't exist.
- `app/(app)/people/[groupId]/page.tsx` — group detail (server component): group info, members list, invite form, and group-scoped activity feed. "Back to people" links to `/people`.
