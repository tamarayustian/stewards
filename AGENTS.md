...
- `components/country-select.tsx` — renders the phone-dial-code `<select>` from `COUNTRIES`; consumed by register, `profile-card.tsx`, and `add-expense-form.tsx`. Option keys use `${code}-${label}` (resolves old register/profile-card key divergence).
- `components/currency-select.tsx` — renders the currency `<select>` from `CURRENCIES`; consumed by `currency-card.tsx` and `add-expense-form.tsx`.
