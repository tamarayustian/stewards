...
- `app/(app)/people/page.tsx` — server component listing groups and contacts: a Groups section (with GroupForm and inline ConfirmAction for delete) and a Contacts section (with PeopleForm and inline ConfirmAction for remove). Row actions when `pair.amountOwedToMe.gt(0)`: `RemindButton` (counterparty registered only), `WhatsAppButton` (phone present; precomputed via `getPairDetail`), otherwise an inline `AddPhoneForm`; a `Details` link always goes to `/people/{id}`.
